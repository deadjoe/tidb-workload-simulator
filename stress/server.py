...
class Settings:
    ...
    def __eq__(self, another):
        return hasattr(another, 'tidb_host') and self.tidb_host == another.tidb_host \
                   and hasattr(another, 'tidb_port') and self.tidb_port == another.tidb_port \
                   and hasattr(another, 'tidb_user') and self.tidb_user == another.tidb_user \
                   and hasattr(another, 'tidb_pass') and self.tidb_pass == another.tidb_pass \
                   and hasattr(another, 'tidb_db') and self.tidb_db == another.tidb_db \
    ...
    def get_dict(self):
        return {
                'tidb_host'   : self.tidb_host,
                'tidb_port'   : self.tidb_port,
                'tidb_user'   : self.tidb_user,
                'tidb_pass'   : self.tidb_pass,
                'tidb_db'     : self.tidb_db,
    ...
    def get_client_arguments(self):
        return {
                ...
                'port'      : self.tidb_port,
                'user'      : self.tidb_user,
                'passwd'    : self.tidb_pass,
                'db'        : self.tidb_db,
                'unix_socket' : '/tmp/tidb.sock'
        }
